...
// The save buffer is 16x16 as the pacman bitmap is 16x16
int buff[16][16]; //in show bmp copy fb/bf bufferthing where pacman is going, draw pacman there, then put buffer back in
int oldstartRow; //The row pacman was on
int oldstartCol; //The column pacman was on
int replacePix = 0; 
int putback(){
  int h = 16, w = 16; //Height and Width of Pacman
  int i, j; //Incrementers 
  int x, y; //Will be used the store the old co-ordinates

  // TODO: copy old contents of fb from the buffer
  for (y = 0, i = oldstartRow; i < h + oldstartRow; i++, y++){
    for (x = 0, j = oldstartCol; j < oldstartCol + w; j++, x++){
      fb[i * 640 + j] = buff[y][x]; //copy old contents of fb from the buffer. fb[i*640*j] is one pixel 
    }
  }
}
//Checks the pixel area is white, and thus is a space you can walk on.
int isWhite(){
//TODO:
}
int show_bmp(char *p, int startRow, int startCol)
{
  ...
  for (yy = 0, i = startRow; i < h + startRow; i++, yy++)
  {
    ...
    for (xx = 0, j = startCol; j < startCol + w; j++, xx++)
    {
      ...
      // TODO:
      buff[yy][xx] = fb[i*640+j];// save old fb value before modification
